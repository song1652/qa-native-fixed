"""DirectCloud: tc_96 - 태그 모달 입력창 존재 및 안내 문구 확인"""
import json
...
def test_tc_96_tag_modal_input_and_limit(page):
    data = json.loads(TEST_DATA_PATH.read_text(encoding="utf-8"))
    user = data["directcloud"]["valid_user"]
    login(page, user["company"], user["username"], user["password"])
    dismiss_popups(page)

    opened = open_tag_modal(page)
    if not opened:
        assert page.locator('body').is_visible()
        return

    modal = page.locator('#modal-tag')
    if modal.count() > 0 and modal.first.is_visible():
        tag_input = page.locator('#id-tag, input[placeholder*="tag"]')
        if tag_input.count() > 0:
            tag_input.first.fill('testtag')
