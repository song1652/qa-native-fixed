"""DirectCloud: tc_203 - 휴지통 파일 복구 확인 다이얼로그 표시 확인"""
import json
...
def test_tc_203_trash_restore_confirm_dialog(page):
    ...
    if trash.count() > 0:
        trash.first.click()
        try:
            page.wait_for_url("**/trash**", timeout=20000)
        except Exception:
    ...
    if file_item.count() > 0:
        try:
            ...
            restore_btn = page.locator(':text("복구"), :text("Restore")')
