"""DirectCloud: tc_45 - 파일 우클릭 → 이동 메뉴 항목 확인"""
import json
...
def test_tc_45_context_menu_move(page):
    """파일 우클릭 → 이동 메뉴 항목 확인"""
    ...
    if file_item.count() > 0:
        try:
            ...
            return
        # 컨텍스트 메뉴 항목 — DOM 확인: "이동"
        move_item = page.locator('li:has-text("이동"), [class*="move"]')
        assert move_item.count() > 0, "이동 컨텍스트 메뉴 항목이 표시되지 않습니다"
    else:
        assert page.locator('body').is_visible()
