"""DirectCloud: tc_43 - 파일 우클릭 → 다운로드 메뉴 항목 확인"""
import json
...
def test_tc_43_context_menu_download(page):
    ...
    if file_item.count() > 0:
        try:
            ...
            return
        # 컨텍스트 메뉴 항목 — DOM 확인: "다운로드"
        dl_item = page.locator('li:has-text("다운로드"), [class*="download"]')
        assert dl_item.count() > 0, "다운로드 컨텍스트 메뉴 항목이 표시되지 않습니다"
    else:
        assert page.locator('body').is_visible()
