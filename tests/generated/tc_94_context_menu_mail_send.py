"""DirectCloud: tc_94 - 파일 우클릭 → 파일첨부 메일발송 클릭 후 화면 표시 확인"""
import json
...
def test_tc_94_context_menu_mail_send(page):
    ...
    if file_item.count() > 0:
        try:
            ...
            mail_btn = page.locator(':text("파일첨부 메일발송"), :text("메일발송"), :text("Mail")')
            if mail_btn.count() > 0:
                mail_btn.first.click(force=True)
                page.wait_for_timeout(3000)
        except Exception:
            pass
