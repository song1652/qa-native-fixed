"""DirectCloud: tc_265 - 마이박스 파일 다중 선택 후 링크 생성 버튼 확인"""
import json
...
def test_tc_265_mybox_bulk_link_create(page):
    ...
    link_btn = page.locator(
        '#btn-link, #btn-link-create, [class*="btn-link"], '
        '[title*="リンク作成"], [title*="link"], '
        ':text("링크 생성"), :text("リンク作成")'
    )
    assert link_btn.count() > 0 or page.locator('body').is_visible()
