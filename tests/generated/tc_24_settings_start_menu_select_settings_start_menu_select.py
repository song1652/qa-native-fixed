"""DirectCloud: tc_24 - 설정 시작메뉴 select"""
import json
...
def test_tc_24_settings_start_menu_select(page):
    ...
    for sel in selects:
        if sel.is_visible():
            all_options.extend(sel.locator('option').all_text_contents())

    combined = ' '.join(all_options)
    expected = ['최근파일', 'Home', 'My Box', 'Shared Box']
    assert any(menu in combined for menu in expected), \
        f"시작메뉴 옵션에 예상 항목이 없습니다: {combined}"
