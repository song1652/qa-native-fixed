"""DirectCloud: tc_49 - 파일 우클릭 → 태그 메뉴 항목 확인"""
import json
...
def test_tc_49_context_menu_tag(page):
    """파일 우클릭 → 태그 메뉴 항목 확인"""
    ...
    if file_item.count() > 0:
        try:
            ...
            return
        # 컨텍스트 메뉴 항목 — DOM 확인: "태그"
        tag_item = page.locator('li:has-text("태그"), [class*="tag"]')
        assert tag_item.count() > 0, "태그 컨텍스트 메뉴 항목이 표시되지 않습니다"
    else:
        assert page.locator('body').is_visible()
