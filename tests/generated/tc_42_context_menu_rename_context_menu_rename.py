"""DirectCloud: tc_42 - 파일 우클릭 → 이름바꾸기 메뉴 항목 확인"""
import json
...
def test_tc_42_context_menu_rename(page):
    ...
    if file_item.count() > 0:
        try:
            ...
            return
        # 컨텍스트 메뉴 항목 — DOM 확인: "이름변경" (이름바꾸기 아님)
        rename_item = page.locator('li:has-text("이름변경"), [class*="rename"]')
        assert rename_item.count() > 0, "이름변경 컨텍스트 메뉴 항목이 표시되지 않습니다"
    else:
        assert page.locator('body').is_visible()
