"""DirectCloud: tc_47 - 파일 우클릭 → 링크공유 메뉴 항목 확인"""
import json
...
def test_tc_47_context_menu_link_share(page):
    ...
    if file_item.count() > 0:
        try:
            ...
            return
        # 컨텍스트 메뉴 항목 — DOM 확인: "링크생성" (링크공유 아님)
        share_item = page.locator('li:has-text("링크생성"), [class*="link"]')
        assert share_item.count() > 0, "링크생성 컨텍스트 메뉴 항목이 표시되지 않습니다"
    else:
        assert page.locator('body').is_visible()
