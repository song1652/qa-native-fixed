"""DirectCloud: tc_46 - 파일 우클릭 → 삭제 메뉴 항목 확인"""
import json
...
def test_tc_46_context_menu_delete(page):
    """파일 우클릭 → 삭제 메뉴 항목 확인"""
    ...
    if file_item.count() > 0:
        try:
            ...
            return
        # 컨텍스트 메뉴 항목 — DOM 확인: "삭제"
        del_item = page.locator('li:has-text("삭제"), [class*="delete"]')
        assert del_item.count() > 0, "삭제 컨텍스트 메뉴 항목이 표시되지 않습니다"
    else:
        assert page.locator('body').is_visible()
