"""DirectCloud: tc_296 - 마이박스 — 우측 정보 패널 태그 목록 표시 확인"""
import json
...
def test_tc_296_mybox_right_panel_file_tags(page):
    ...
    if file_row.count() > 0:
        ...
        # Look for right panel with tag info
        tag_section = page.locator(
            '[class*="tag"], [class*="Tag"], .info-panel [class*="tag"], '
            '.right-panel [class*="tag"], [class*="fileTag"], [class*="file-tag"], '
            'label:has-text("タグ"), label:has-text("Tag"), '
            'span:has-text("タグ"), dt:has-text("タグ")'
        )
        if tag_section.count() > 0:
            assert tag_section.first.is_visible()
        else:
            # Right panel may be present even without tag section visible
