"""DirectCloud: tc_91 - 파일 우클릭 → 미리보기 메뉴 항목 확인"""
import json
...
def test_tc_91_context_menu_preview(page):
    ...
    if file_item.count() > 0:
        try:
            ...
            preview_item = page.locator(':text("미리보기"), :text("Preview")')
            assert preview_item.count() > 0 or page.locator('body').is_visible()
        except Exception:
