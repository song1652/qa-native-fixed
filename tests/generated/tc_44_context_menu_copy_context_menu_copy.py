"""DirectCloud: tc_44 - 파일 우클릭 → 복사 메뉴 항목 확인"""
import json
...
def test_tc_44_context_menu_copy(page):
    """파일 우클릭 → 복사 메뉴 항목 확인"""
    ...
    if file_item.count() > 0:
        try:
            ...
            return
        # 컨텍스트 메뉴 항목 — DOM 확인: "복사"
        copy_item = page.locator('li:has-text("복사"), [class*="copy"]')
        assert copy_item.count() > 0, "복사 컨텍스트 메뉴 항목이 표시되지 않습니다"
    else:
        assert page.locator('body').is_visible()
