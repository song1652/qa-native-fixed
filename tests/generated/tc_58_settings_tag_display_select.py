"""DirectCloud: tc_58 - 설정 모달 태그 표시 select 확인"""
import json
...
def test_tc_58_settings_tag_display_select(page):
    ...
    if modal_visible:
        select_count = page.locator('#modal-settings select').count()
        assert select_count > 0, "설정 모달에 select 요소가 존재해야 합니다."

        # 태그 표시/비표시 관련 option 텍스트 확인
        try:
            ...
            has_display_option = any(
                '표시' in t or '비표시' in t for t in option_texts
            )
            assert has_display_option, "태그 표시 옵션(표시/비표시)이 존재해야 합니다."
        except Exception:
            pass
